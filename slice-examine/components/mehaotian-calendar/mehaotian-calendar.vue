							...
							<view class="calender-body-date-week">
								<block v-for="(day , index) in weeks" :key="index">
									...
									:class="{
										...
										'date-current':(day.date==canlender.date||day.checked) && canlender.month == day.month,
										'lunar':lunar}"
									 @tap='selectDays(week,index,canlender.month === day.month ,day.disable)'>
										{{day.date}}
										<view v-if="lunar" class="lunar">{{day.lunar}}</view>
										<view v-if="day.have" class="data-circle">
											<!--checkType:1、超期完成 2、超期 3、完成-->
											<image :src="day.checkType ==1?'../../static/img/jiaobiao-yellow.png':day.checkType ==2?'../../static/img/jiaobiao-red.png':day.checkType ==3?'../../static/img/jiaobiao-green.png':'../../static/img/jiaobiao-gray.png'"/>
										</view>
										<view v-if="day.have" class="data-img">
											<block v-if="(day.date==canlender.date||day.checked) && canlender.month == day.month">
												<image src="../../static/img/data-img-click.png" />
											</block>
											<block v-else>
												<image src="../../static/img/data-img.png" />
											</block>
										</view>
									</view>
								</block>
