...
import com.test.weixin.utils.HttpClientConnectionManager;
import com.test.weixin.utils.MD5Util;
import com.test.weixin.utils.WxSign;

/**
 * 
 * @ClassName: YSSPSmallPaaWeChatUtil
 * @Description: 艺术私品小程序微信支付相关封装工具类(这里用一句话描述这个类的作用)
 * @author acai
 * @date 2018年9月10日 下午2:37:21
 *
 */
public class YSSPSmallPaaWeChatUtil {
	private static Logger log = LoggerFactory.getLogger(YSSPSmallPaaWeChatUtil.class);
	// HTTP请求器
	...
	public static String unifiedorder(String body, String out_trade_no, Integer total_fee, String ip, String notify_url,
		...
		parameters.put("appid", SmallAppConstant.AppID);
		parameters.put("body", body);
		parameters.put("mch_id", SmallAppConstant.MCH_ID);
		...
		parameters.put("trade_type", SmallAppConstant.Trade_Type);
		String sign = WxSign.createSign(parameters, SmallAppConstant.API_KEY);
		...
		sb.append("<xml>");
		setXmlKV(sb, "appid", SmallAppConstant.AppID);
		setXmlKV(sb, "body", body);
		setXmlKV(sb, "mch_id", SmallAppConstant.MCH_ID);
		...
		setXmlKV(sb, "trade_type", SmallAppConstant.Trade_Type);
		...
		HttpPost httppost = new HttpPost(SmallAppConstant.Unified_Order_URL);
	...
	public static String refundorder(String transaction_id, int total_fee, int refund_fee, FileInputStream instream)
		...
		try {
			keyStore.load(instream, SmallAppConstant.MCH_ID.toCharArray());
		} finally {
			instream.close();
		}

		SSLContext sslcontext = SSLContexts.custom().loadKeyMaterial(keyStore, SmallAppConstant.MCH_ID.toCharArray()) // 加载证书密码，默认为商户ID
				.build();
		...
		// 设置签名
		String sign = WXPayUtil.generateSignature(parameters, SmallAppConstant.API_KEY);
		...
		/*----5.发送数据到微信的退款接口---- */
		HttpPost httpost = HttpClientConnectionManager.getPostMethod(SmallAppConstant.refund_URL);
